...
handleToggle = (id) => {
     ...
     const Faq = this.props.data.allWordpressWpFaq;
      return (
       <>
               ...
               <h2 className="section-heading font-weight-semi-bold mb-4 text-center">Frequently Asked Questions</h2>
               ...
               <Accordion defaultActiveKey="0">
                                 ...
                                 {prop.node.acf.faq.map(
									 ...
									 <Card.Header  className="border-0">
                                         <Accordion.Toggle onClick={() => {this.handleToggle(i)}} className="label-text border-0  mb-0 "   as={Card.Title} eventKey={i}>
                                           <h3 className="label-text-2 text-dark font-weight-regular mb-0" >{prop1.question}</h3> <span className="accrodian-icon"><i className="fa fa-plus"></i></span>
                                         </Accordion.Toggle>
                                       </Card.Header>
   ...
   const Faq = () => (
     ...
     query={graphql`
       query {
        allWordpressWpFaq(filter: {title: {eq: "FAQ"}}) {
          edges {
            node {
              acf {
                faq {
