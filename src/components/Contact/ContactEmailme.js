      ...
      <>
						...
						<div className="input-group">
							...
							<div className="input-group-append">
								<button disabled={this.state.IsSubmit}  onClick={this.submitForm} className="btn btn-app-download-outline rounded" type="button">{ this.state.IsSubmit ? <div className="spinner-border" role="status"><span className="sr-only">Loading...</span></div> : <span>Email Me</span> }</button>
							</div>
						</div>	
							
							<p className="mb-4 mt-1 " >{this.validator.message('Email', this.state.email, 'required|email')}</p>
							<div class={"alert alert-success text-success bg-white "+this.state.shown}>YupiT download link has been sent to your E-mail.
								</div>
	 </>
    )
  }
