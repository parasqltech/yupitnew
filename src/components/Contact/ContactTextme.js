      ...
      <>		
				...
				<div className="input-group ">
                                            ...
                                            <div className="input-group-append">
                                                <button disabled={this.state.IsSubmit}  onClick={this.submitForm} className="btn btn-app-download rounded" type="button">{ this.state.IsSubmit ? <div className="spinner-border text-light" role="status"><span className="sr-only">Loading...</span></div> : <span>Text Me</span> }</button>
                                            </div>
                                        </div>			
							<input type="hidden" name="url" id="url" />
							<input  type="hidden" value="contactTextme" name="form_name"/>
							<p className="mb-3 mt-1" >{this.validator.message('Mobile Number', this.state.number, 'required|numeric|min:9|max:13')}
							</p>
							<div class={"alert alert-success text-success bg-white "+this.state.shown}>YupiT download link has been sent to your mobile no.
								</div>
    </>
    )
  }
